...
import argparse, math, os, random, subprocess, sys, tempfile, json
...
def main() -> None:
    ...
    for i in range(1, N + 1):
        if i == N:
            lengths.append(TARGET_SEC - s); break
        left  = N - i
        rem   = TARGET_SEC - s
    ...
    for idx, L in enumerate(lengths, 1):
        ...
        file_idx = next(j for j, p in enumerate(prefix[:-1])
                        if prefix[j] <= global_start < prefix[j+1])
        src      = files[file_idx]
        local_start = global_start - prefix[file_idx]

        out_clip = tmp_dir / f"clip{idx:03d}.mkv"
        logger.info(f"[{idx:03d}] {src.name:20s}  seek={local_start:7.2f}s  len={L:2d}s")

        subprocess.run(
            ["ffmpeg", "-hide_banner", "-loglevel", "error", "-y",
             "-ss", f"{local_start:.6f}", "-i", str(src), "-t", str(L),
             "-fflags", "+genpts", "-reset_timestamps", "1",
             "-video_track_timescale", "90000",
             "-c", "copy", "-movflags", "+faststart", str(out_clip)],
            check=True,
        )
        ...
        if usable_total > TARGET_SEC + 0.0001:       # overshoot → trim last clip
            ...
            subprocess.run(
                ["ffmpeg", "-hide_banner", "-loglevel", "error", "-y",
                 "-ss", f"{local_start:.6f}", "-i", str(src), "-t", f"{new_len:.6f}",
                 "-fflags", "+genpts", "-reset_timestamps", "1",
                 "-video_track_timescale", "90000",
                 "-c", "copy", "-movflags", "+faststart", str(out_clip)],
                check=True,
